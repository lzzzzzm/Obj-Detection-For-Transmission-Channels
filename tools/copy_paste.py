...
def is_coincide(bbox_1, bbox_2):
    ...
    '''
    if (bbox_1[0] >= bbox_2[0] + bbox_2[2]) or (bbox_1[0] + bbox_1[2] <= bbox_2[0]) or (
            bbox_1[1] + bbox_1[3] <= bbox_2[1]) or (bbox_1[1] >= bbox_2[1] + bbox_2[3]):
        return False
    else:
        return True


def img_add(img_src_path, img_main_path, img_out_path, bbox_src):
    '''
    ...
    for bboxi in bbox_src:
        ...
        # print(img_main.size)
        # 裁剪后保存并覆盖原来的main图像
        img_main.save(img_out_path)


def copy_paste(img_main_path, img_out_path, img_src_path, main_anns, src_anns, coincide=False, muti_obj=False):
    """
    整个复制粘贴操作
    ...
    # 将从src裁剪的区域加到main中
    img_add(img_src_path, img_main_path, img_out_path, bbox_src)
    # 粘贴成功标志位
    flag = True
...
def main(args):
    ...
    with tqdm.tqdm(list_imgIds, colour='red') as tbar:  # 设置进度条
        for imgId in tbar:
            ...
            vis_img = cv.imread(img_main_path)
            ...
            img_out_path = os.path.join(args.image_out_path, main_img['file_name'])
            # 数据增强（copy-paste）
            flag, bbox_src = copy_paste(img_main_path, img_out_path,img_src_path, main_anns,
...
def get_args():
    ...
    parser.add_argument("--input_dir", default="../dataset/channel_transmission/train_images", type=str,
                        help="要进行数据增强的图像路径")
    parser.add_argument("--image_out_path", default="../dataset/channel_transmission/train_copypaste_images", type=str,
                        help="copypaste后图片保存地址")
    parser.add_argument("--json_path", default="../dataset/channel_transmission/annotations/train.json", type=str,
                        help="要进行数据增强的图像对应的总的json文件")
    parser.add_argument("--output_json", default="../copy_paste_train.json", type=str,
    ...
    parser.add_argument("--category_id", default=1, type=int,
...
if __name__ == "__main__":
    args = get_args()
    if not os.path.exists(args.image_out_path):
        os.mkdir(args.image_out_path)
    main(args)
